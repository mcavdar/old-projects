...
(function($, videojs, undefined) {
   ...
   function registerVideoJsMarkersPlugin(options) {
      ...
      function addMarkers(newMarkers) {
         ...
         $.each(newMarkers, function(index, marker) {
            ...
            marker.div.on('click', function(e) {
		     ...
		     player.pause();
			//$.featherlight('http://maps.google.com/maps?q=loc:'+markers[overlayIndex].lng+","+markers[overlayIndex].lat);
            });
	 ...
	 var currentTime = player.currentTime();
		...
		var showme = document.getElementById("vjs-logobrand-image");
			showme.style.visibility = "visible";
		
		var alink = document.getElementById("vjs-logobrand-image-destination");
			alink.href = 'http://maps.google.com/maps?q='+markers[overlayIndex].text+'&t=k&loc:'+markers[overlayIndex].lng+','+markers[overlayIndex].lat;
		var imag = document.getElementById('vjs-logobrand-image');
			imag.src = "https://maps.googleapis.com/maps/api/staticmap?center="+markers[overlayIndex].lng+","+markers[overlayIndex].lat+"&zoom=11&size=250x125&maptype=roadmap&markers=color:blue%7C"+markers[overlayIndex].lng+","+markers[overlayIndex].lat;
		...
		showme.style.visibility = "hidden";
	       ...
	       //logobrand.css('visibility', "visible");
            }
         }
      }
      function myFunction() {
	...
	var showmee = document.getElementById("vjs-logobrand-image");
		showmee.style.visibility = "hidden";

         breakOverlay = $("<div class='vjs-break-overlay'><div class='vjs-break-overlay-text'></div></div>")
            .css(setting.breakOverlay.style);
         videoWrapper.append(breakOverlay);
         overlayIndex = -1;
         breakOverlay.on('click', function(e) {
            window.open("http://maps.google.com/maps?q="+markers[overlayIndex].text+"&t=k&loc:"+markers[overlayIndex].lng+","+markers[overlayIndex].lat);
         });
      }
