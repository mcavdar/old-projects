...
/**
 * Impl�mente le parcours d'arbre en largeur d'abord pour un thread.
 */
public class BreadthFirstRunnable implements Runnable { // d�lai entre deux
														// it�rations
	 ...
	 *            arbre de recherche � construire
	 */
	public BreadthFirstRunnable(SearchTreeFrame frame, SearchTree tree) {
		this.frame = frame;
		this.tree = tree;
		tree.init();
	}
	...
	/**
	 * Applique la parcours d'arbre en largeur d'abord.
	 */
	public void makeTree() { // TODO � compl�ter par l'�tudiant
		...
		fifo.offer(tree.getRoot());
		int counter = 1;
		while (!fifo.isEmpty() && solution == null) {
			SearchNode node = fifo.poll();
