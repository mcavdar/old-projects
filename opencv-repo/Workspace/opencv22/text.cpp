...
int main(int argc, const char * argv[]) {
	...
	cout << "Demo program of the Extremal Region Filter algorithm described in "
			<< endl;
	cout
			<< "Neumann L., Matas J.: Real-Time Scene Text Localization and Recognition, CVPR 2012"
			<< endl << endl;
...
// Create ERFilter objects with the 1st and 2nd stage default classifiers
	Ptr<ERFilter> er_filter1 = createERFilterNM1(
			loadClassifierNM1("trained_classifierNM1.xml"), 16, 0.00015f, 0.13f,
			0.2f, true, 0.1f);
...
// Detect character groups
	...
	vector<Rect> groups;
	erGrouping(channels, regions, "trained_classifier_erGrouping.xml", 0.5,
			groups);
...
void groups_draw(Mat &src, vector<Rect> &groups) {
	for (int i = (int) groups.size() - 1; i >= 0; i--) {
		if (src.type() == CV_8UC3)
			...
			rectangle(src, groups.at(i).tl(), groups.at(i).br(), Scalar(255), 3,
					8);
	}
}
